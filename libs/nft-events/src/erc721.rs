    ...
    loop {
        match evm_client.get_latest_block_number().await {
            Ok(latest_block_number) => {
                ...
                if to >= from {
                    debug!("Scan for {} ERC721 events in block range of {} - {}({})", evm_client.chain_name, from, to, to - from + 1);
                    let start = Instant::now();
                    match erc721_evm::get_erc721_events(&evm_client, from, to).await {
                        Ok(events) => {
                            info!("{} {} ERC721 events were scanned in block range of {} - {}({})", events.len(), evm_client.chain_name, from, to, to - from + 1);
                            for event in events {
                                ...
                                match get_metadata(evm_client, db_conn, &event).await {
                                    Ok(metadata) => {
                                        if let Err(err) = callback.on_erc721_event(event.clone(), metadata.0, metadata.1, metadata.2).await {
                    ...
                    debug!("Track {} ERC721 events too fast, wait for 30 seconds.", evm_client.chain_name);
                    sleep(Duration::from_secs(30)).await;
                }
...
async fn get_metadata(evm_client: &EvmClient, db_conn: &Connection, event: &Erc721Event) -> Result<(Option<String>, Option<String>, Option<String>)> {
    save_metadata_to_db_if_not_exists(evm_client, db_conn, &event.address, &event.token_id).await?;
    let collection = erc721_db::get_collection_from_db(db_conn, &format!("{:?}", event.address))?.unwrap();
    let token = erc721_db::get_token_from_db(db_conn, collection.0, &event.token_id.to_string())?.unwrap();
...
async fn save_metadata_to_db_if_not_exists(evm_client: &EvmClient, db_conn: &Connection, address: &H160, token_id: &U256) -> Result<()> {
    ...
    let collection_id = if let Some(collection) = erc721_db::get_collection_from_db(db_conn, &address_string)? {
        collection.0
    } else {
        if let Some((name, symbol)) = evm_client.get_erc721_name_symbol(address).await? {
            erc721_db::add_collection_to_db(db_conn, address_string.clone(), Some(name), Some(symbol))?
        } else {
