...
pub fn get_collection_from_db(conn: &Connection, address: &str) -> Result<Option<(usize, String)>> {
    let sql = format!(
        "SELECT * from erc1155_collections where address='{}'",
        address
    );
    let mut stmt = conn.prepare(sql.as_str())?;

    match stmt.query_row([], |row| Ok((row.get(0)?, row.get(1)?))) {
...
pub fn get_token_from_db(
    ...
    let sql = format!(
        "SELECT * from erc1155_tokens where collection_id={} and token_id='{}'",
...
mod tests {
    ...
    async fn test_get_collection_from_db() {
        ...
        let result = get_collection_from_db(&conn, &address).unwrap();
        assert_eq!(
            Some((1, "0x0000000000000000000000000000000000000000".to_string())),
            result
        );
    ...
    async fn test_get_token_from_db() {
        ...
        // add the token
        let token_uri =
            Some("ipfs://ipfs/QmXTSZ7ag9yzQFkvUrZ5qr7KuCUtW2jeDiYJrJ8s7TpSNb".to_owned());
        let id =
